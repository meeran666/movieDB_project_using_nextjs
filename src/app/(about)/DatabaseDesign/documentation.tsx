...
return result;
  ...
  return (
    <>
      <div className="pb-20 text-4xl font-bold">Database Design</div>
      <div className="pb-7">
        Database design is a crucial part of this website which serves priority
        data to the frontend. Postgres database is used in backend which is
        faster then any other other database.
      </div>
      <div className="pb-15">
        ...
        {` '/'`} route. main table, genre table and abbreviation table are used
        for detail page in {`'/detail'`} route.
      </div>
      <div className="pb-7 text-3xl font-bold">Tables</div>
      <div className="pb-7">
        Main table contain 23 fields with there data type which are id (integer,
        primaryKey, notNull), title (varchar), voteAverage (text), voteCount
        (text), status: (text), releaseDate: (text), revenue (bigint), runtime
        integer, adult boolean, backdropPath (text), budget (bigint), homepage
        (text), imdbId (text), languageCode (varchar), originalTitle (text),
        overview (text), popularity (text), posterPath (text), tagline (text),
        productionCompanies (text), productionCountries (text), spokenLanguages
        (text), keywords: (text),
      </div>
      <div className="pb-7">
        ...
        (boolean), history (boolean), tvMovie (boolean), documentary (boolean),
      </div>
      <div className="pb-15">
      ...
      <div className="pb-7 text-3xl font-bold">Handle Database in Backend</div>
      <div className="pb-7">
        ...
        drizzle orm with typescript. Drizzle orm checks structure and data types
        of all the api response from database server, it also simplify the
        writing of query.
      </div>
      <div className="pb-15">
        Before execution of Any query of any route a trigger is setup which will
        take the value(parameter) from html and add it in URL as query
        parameter. There is a POST request handler sit on backend which extract
        the query parameters from URL for every request from frontend.Query
        paremeter contains movie title, Query parameter then send to function
        which handle the connection of database and query to database. The code
        which handle this query is put inside the try catch statement for
        catching any error coming from database server during the execution of
        query.
      </div>
      <div className="pb-7 text-3xl font-bold">Movie List Query</div>
      <div className="pb-7">
        ...
        statement is used for selecting the id , title , release date columns of
        table.
      </div>
      <div className="pb-7">
      ...
      <div className="pb-7">
        {`'leftjoin()', `} this keyword is used for joining the main table and
        abbreviation table with the common column is {`' languageCode'`}.
      </div>
      <div className="pb-7">
        {`'innerJoin()', `} this keyword is used for joining the main table and
        genre table with the common column{`' id'`}.
      </div>
      ...
      <div className="pb-7">
        ...
        fields from the table.Where() keywords specify the conditional clase of
        title of main table contains wrapped_title and id of main table not
        equal to id(query parameter).Movie title is in order of alphabatical in
        decreasing.
      </div>
      <div className="pb-15">
        <SyntaxHighlighter
