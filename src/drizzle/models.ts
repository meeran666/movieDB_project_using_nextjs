...
export const abbreviationTable = pgTable(
  "abbreviation_table",
  {
    originalLanguage: varchar("original_language", { length: 70 })
      .primaryKey()
      .notNull(),
  ...
  (table) => [
    check(
      "abbreviation_table_original_language_not_null",
      sql`NOT NULL original_language`,
    ),
  ],
);
...
export const genreTable = pgTable(
  ...
  (table) => [check("genre_table_id_not_null", sql`NOT NULL id`)],
);
